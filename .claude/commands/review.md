...
以下の数値は**制限ではなく、プロジェクト規模の目安**として参照してください。

#### L1 ビジョン・要求

| 項目 | 目安 | 規模感 |
|------|------|--------|
| REQ数 | 5-50個 | 5個未満: 小規模ツール / 50個超: 大規模システム |
| REQ記述量 | 1REQ = 50-300文字 | 短い: シンプルな要件 / 長い: 複雑な要件 |
| NFR数 | 3-15個 | 少ない: 内部ツール / 多い: 品質重視システム |
...
| 項目 | 目安 | 規模感 |
|------|------|--------|
| フェーズ数 | 2-8個 | 2-3個: 小規模 / 5-8個: 中〜大規模 |
| 機能数（F） | 10-100個 | 10-30個: 小規模 / 50-100個: 大規模 |
| REQカバレッジ | 100% | すべてのREQがF/NFに紐付いているか（必須） |
| 孤立F | 0個 | どのPHにも属さないFがないか（必須） |
| 循環依存 | 0個 | F間の依存関係に循環がないか（必須） |

#### L3 機能ドキュメント

| 項目 | 目安 | 備考 |
|------|------|------|
| 受け入れ条件数 | 1-10個 | 最低1個は必須。複雑な機能は多くなる |
| タスク数 | - | AIが実装時に生成するため、事前記載は任意 |
| TODO残存 | 0個（reviewed時） | draft時は許容、reviewed時は0 |
| req_ids | 1個以上 | どのREQにも紐付かないFは要確認（必須） |
| phase | 必須 | 未指定は要確認（必須） |
