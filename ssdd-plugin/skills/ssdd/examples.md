...
> **重要**: Claude 4は例の詳細に非常に敏感です。以下の「良い例」のパターンに従い、「避けるべき例」のパターンを避けてください。
...
単純なCRUD機能に対して過剰な詳細を記載している例：

```yaml
# 悪い例: 単純な「ユーザー一覧表示」機能に対して...
---
id: F-20250125-099
kind: feature
# ...
...
## 2. データモデル
### 2.3 状態遷移  ← 不要：一覧表示に状態遷移はない
（無理やり状態遷移図を記載）

## 3. インターフェース
### 3.4 シーケンス  ← 不要：単純なGETリクエストにシーケンス図は過剰
（複雑なシーケンス図を記載）

## 5. 受け入れ条件
### AC-1 〜 AC-15  ← 過剰：一覧表示に15個の受け入れ条件は多すぎる
```

**問題点**:
- テンプレートの全セクションを無理に埋めている
- 機能の複雑さに見合わない詳細度

### ❌ 過剰なフェーズ分割（悪い例）

```yaml
# 悪い例: 小規模プロジェクト（機能5個）を10フェーズに分割
PH-001: ログイン機能
PH-002: ログアウト機能  ← 不要：ログインと一緒でよい
PH-003: パスワードリセット
PH-004: メール認証  ← 不要：細かすぎる
PH-005: ...
```

**問題点**:
- フェーズが細かすぎて管理コストが高い
- 各フェーズで「デモ可能な成果物」が出せない
...
| 項目 | 選定技術 |
|------|---------|
| フロント | Next.js + React Query + Zustand + Jotai |  ← 過剰
| バック | NestJS + TypeORM + Redis + Elasticsearch |  ← 過剰
| インフラ | Kubernetes + Istio + ArgoCD |  ← 過剰
| 監視 | Prometheus + Grafana + Jaeger |  ← 過剰
```
