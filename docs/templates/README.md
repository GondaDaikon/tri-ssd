...
複数プラットフォームにまたがる製品（例: Electron、React Native、Flutter）では、以下の方針を推奨：

| 製品タイプ | 推奨アプローチ |
|-----------|---------------|
| **Electron（Web + Desktop）** | 主要機能はWeb用テンプレート、OS固有機能（トレイ、ファイルアクセス等）はDesktop用テンプレートを個別に使用 |
| **React Native / Flutter（Web + Mobile）** | 共通機能はWeb用テンプレート、デバイス機能（カメラ、プッシュ通知等）はMobile用テンプレートを個別に使用 |
| **CLI + GUI併用** | CLIとGUI機能を別々のL3として管理し、それぞれ適切なテンプレートを使用 |

**判断基準**:
- 機能の主要なインタラクションがどのプラットフォームで発生するか
- プラットフォーム固有の考慮事項（ストア申請、OS権限等）が必要か
