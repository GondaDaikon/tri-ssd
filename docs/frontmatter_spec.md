...
Tri-SSDドキュメントのフロントマター仕様を定義します。

## ドキュメント種別

| 種別 | 用途 | 生成コマンド |
|------|------|-------------|
| L1 Vision | ビジョン・要求ドキュメント | `/draft-l1` |
| L2 Foundation | 技術基盤（用語集・技術方針・アーキテクチャ・NFR） | `/draft-l2` |
| L2 Phases | フェーズ定義・機能一覧 | `/gen-phases` |
| L2 Rules | 実装ルール（コード生成制約） | `/draft-rules` |
| L3 Feature | 機能ドキュメント | `/gen-l3` |

> **Note**: 各コマンドに出力フォーマットが埋め込まれており、正しいYAMLフロントマター形式でドキュメントを生成します。ドメイン固有の考慮事項はL2の技術方針・NFRカタログで定義し、L3生成時にAIがコンテキストに応じて適切な内容を生成します。
...
**重要**: `title`フィールドは廃止されました。本文の最初の`# 見出し`がドキュメントのタイトルとして扱われます。これにより、フロントマターと本文の同期問題を根本的に解決します。
...
| foundation | L2技術基盤（用語集・技術方針・アーキテクチャ・NFR） |
| phase | フェーズ定義・機能一覧（L2） |
| rules | 実装ルール（L2、コード生成制約） |
| feature | 機能ドキュメント（L3） |

> **Note**: 要件（REQ-xxx）はL1ビジョン内でインライン定義、非機能要求（NF-xxx）はL2技術基盤内で定義します。
