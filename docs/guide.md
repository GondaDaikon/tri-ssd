...
Tri-SSD の概念・設計方針・三層モデルの解説です。

## 1. 三層モデル（L1/L2/L3）

### 1.1 三層構造の概要

Tri-SSD におけるドキュメントは、次の三層で構成されます。
...
Tri-SSDの核心は、**複雑な要件を機能単位に分割し、AIが扱える粒度に統治する**ことにあります。
...
- `/draft-l1`, `/draft-l2`, `/gen-phases`, `/draft-rules`, `/gen-l3` コマンドが自動採番
...
**運用方針**:
- **Tri-SSDではフェーズを定義**: L2でフェーズ（PH-YYYYMMDD-nnn）を管理
...
L2には、AI/人間がコード実装時に守るべきルールを定義する`rules.md`を配置できます。

#### 目的・役割

- **コード生成時の制約**: AIがL3を元にコード生成する際に参照する制約・ルール
- **プロジェクト全体の一貫性**: 命名規則、アーキテクチャ、コーディング規約を統一
- **ドメインルール**: ビジネス上の絶対ルール（L1から抽出されるもの）
- **L3実装フィードバックの蓄積**: 実装中に発見した汎用的な問題・ルールを追記

#### rules.md の構成

1. **コード規約**: 言語・命名・ディレクトリ構成
2. **アーキテクチャ**: レイヤー構成・依存方向・状態管理
3. **ドメインルール**: 絶対に壊してはいけないビジネスルール
4. **API・データアクセス**: API規約・DBアクセスルール
5. **セキュリティ・ログ**: 認証・認可・ログ方針
6. **テスト**: テスト戦略・命名・カバレッジ方針
7. **AI固有の禁止事項**: AIがやりがちなミスへの制約

#### 生成と更新

- `/draft-rules` コマンドで初期版（たたき台）を生成
- L3実装中に発見した問題は、汎用的・致命的であればrules.mdに追記
- 追加判断基準: 再発可能性 × 致命度 × 汎用性
...
コード変更時に、どのドキュメントを更新すべきか判断する基準です。
...
- フェーズ案の提示
- 非機能カタログ案の起草
- **技術スタック候補の提示**: AIが候補を提示し、ユーザーとの対話を通じて人間が選択
