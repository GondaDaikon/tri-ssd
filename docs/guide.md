...
SSDD v2.0 では、実運用での課題を解決する重要な変更が行われました：
...
**解決**: AIが候補提示 → 人間が選択（AskUserQuestion使用）

詳細は本ドキュメント各セクションおよび`docs/templates/README.md`を参照。
...
| L3 | **実装前の仕様書**（実装後はコードが正） |
| コード＋テスト | 実装された挙動の最終的な正 |

> L3 は**実装前は仕様書**として機能し、**実装後はコードが正**となります。
> 実装後のL3は、コードとテストを読み解く前の入口となる「解説ビュー」として参照します。
...
> **v2.3変更**: 「ライト版運用」の概念は廃止されました。小規模プロジェクトでも標準SSDDを採用し、段階的に導入することを推奨します。詳細は`docs/checklists.md`の「v2.3変更点: ライト版の廃止」を参照してください。
...
**v2.0変更点**: features_index.mdは廃止され、phases.md内に統合されました。
...
**v2.0変更点**: nfr.mdは廃止され、overview.md内に統合されました。
...
- フェーズ案の提示
- 非機能カタログ案の起草
- **技術スタック候補の提示（v2.0）**: AIが候補を提示し、人間が選択（AskUserQuestion使用）
