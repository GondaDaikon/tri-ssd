...
SSDD v0.1 以降では、運用上の問題点を根本的に解決する4つの重要な変更が行われました。本ドキュメントでは、既存プロジェクトを SSDD の現行バージョンへ移行する方法を説明します。
...
1. **既存IDは保持**: 旧形式のIDをそのまま維持
2. **新規IDは現行形式**: 新しく作成するドキュメントのみ現行形式（タイムスタンプベース）を使用
3. **テンプレート更新**: 新規作成時に ssdd-plugin のテンプレートを使用
...
| 項目 | 旧形式 | 現行形式 |
|------|------|------|
| 形式 | 連番（REQ-0001） | タイムスタンプベース（REQ-20251126-001） |
...
# 新規IDの例
REQ-20251126-001  # 現行形式 - 新規作成時に使用
REQ-20251126-002  # 現行形式 - 新規作成時に使用
```
...
2. マッピング表を作成
   ```
   REQ-0001 → REQ-20251126-001
   REQ-0002 → REQ-20251126-002
   F-0001 → F-20251126-001
   ```

3. 全ドキュメントの参照を一括置換
   - フロントマターの `id` フィールド
   - `req_ids`, `nfr_ids`, `phase` の参照
...
**原因**: 現行仕様（REQ-20251126-001）

**対応方法:**

- **推奨**: 現行形式をそのまま使用（並行開発の安全性を優先）
- **代替案**: プロジェクト固有の短縮形式を定義（例: `REQ-2511-001`）
  - ただし、衝突リスクが増えるため非推奨
...
| Week 2-4 | 段階的にtitle変換、新規ドキュメントは現行形式 |
...
| Phase | タスク |
|-------|--------|
| Phase 1 (Month 1) | テンプレート更新、新規は現行形式のみ |
...
- [SSDD バージョン情報](README.md#バージョン情報)
