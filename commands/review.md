...
4. **昇格完了後**: 気づきの反映を提案（下記参照）

### 昇格時の気づき反映提案

状態昇格の完了後、以下を確認してユーザーに提案する:

**L3 の reviewed → implemented 昇格時**:
- L3 の実装メモ（セクション7）を確認
- 他の機能でも適用すべきルールがあれば rules.md への追加を提案

**draft → reviewed 昇格時**:
- レビュー過程で発見した課題・改善点があれば関連ドキュメントへの反映を提案
...
**他のドキュメントへの反映候補**:
- [ ] rules.md に「[ルール内容]」を追加
- [ ] foundation.md の「[セクション]」を更新

反映しますか？ (yes/no/後で)
```

**注**: 全ての気づきを反映する必要はない。「プロジェクト全体に価値がある」ものだけ選別する。
