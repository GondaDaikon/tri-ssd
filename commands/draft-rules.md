...
`/gen-code` や `/review` で「rules.md への反映」が提案された場合の追記手順:

### 追記の判断基準

| 追記すべき | 追記しない |
|-----------|-----------|
| 他の機能でも適用すべきルール | その機能固有の注意点 |
| プロジェクト全体で統一すべき規約 | 一時的な回避策 |
| 今後も注意が必要な制約 | 既に rules.md にあるルールの重複 |

### 追記方法

1. 該当するセクションを特定（コード規約、ドメインルール等）
2. ルールを追記
3. **更新履歴に追記理由と出典を記録**
...
| 日付 | 変更内容 | 担当 | 追加理由 |
|------|----------|------|----------|
| 2025-01-20 | 3.2 に「日時はUTC保存」を追加 | AI | F-20250120-001 実装時に発覚 |
```
