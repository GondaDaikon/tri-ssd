...
- エディタ設定: DevContainer / 推奨拡張機能 / EditorConfig
- リンター/フォーマッター: ESLint / Prettier / Biome / Ruff
...
### 1.7 CI/CD（省略可）
- CI: GitHub Actions / GitLab CI / CircleCI / なし
- CD: 自動デプロイ / 手動デプロイ / なし
...
<dialogue_policy>
L1を分析し、該当するカテゴリのみ3択+任意形式で技術選定を進める。
各カテゴリでWebSearchして候補を提示。
</dialogue_policy>

1. フロントエンドは？（UIがある場合、検索結果から3候補+その他）
2. バックエンドは？（サーバー処理がある場合、検索結果から3候補+その他）
3. データベースは？（永続化が必要な場合、検索結果から3候補+その他）

→ 回答を受けて深掘り（認証、インフラ等）
→ 新しい情報が出なくなったら Step 3 へ

<!-- 以下に該当する場合のみ追加質問 -->
4. 開発環境の制約は？（チーム統一 / 開発≠本番 / OS指定 / その他）
5. CI/CDは？（GitHub Actions / GitLab CI / 手動のみ / その他）
