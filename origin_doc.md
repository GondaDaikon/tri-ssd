# L2Aサイクル開発フロー運用ガイド（Git＋エージェント版／改訂）

## 1. はじめに

### 1.1 本ガイドの目的

本ガイドは、「二層要件AIアジャイル開発サイクル（L2Aサイクル）」における
...
実務上のフローとアーティファクト構造を再構成した運用フレームが L2A サイクルである。
...
AI を用いない運用は本ガイドの対象外とし、L2A は「AI を前提とした開発フロー」として位置づける。
...
6. レトロスペクティブ
   ...
   * L2A運用（AI出力品質、ドキュメント鮮度、更新コスト）について振り返る
...
本ガイドでは、L2Aサイクルにおけるドキュメント運用と開発フローを
...
* 運用実績に基づく改善（L2A 自体のレトロスペクティブ）

を通じて、L2Aサイクルを「開発者とAIエージェントの共創」に適した運用フレームとして継続的に洗練させていく。
